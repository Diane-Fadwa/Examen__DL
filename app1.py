...
# Validation du JSON fourni au trigger manuel

@task
def validate_rattrapage_payload(payload: dict):
    """
    Valide le JSON passé via dag_run.conf lors du trigger manuel.
    Format attendu :
    {
        "contract_path": "/contracts/client.yml",
        "files": [
    ...
    logger.info("Rattrapage payload validated: contract=%s, %d files", contract_path, len(files))
    return {"contract_path": contract_path, "files": files}

# DAG déclenché manuellement pour le rattrapage

with DAG(
    dag_id="dag_rattrapage",
    description="DAG de rattrapage déclenché manuellement avec payload JSON",
    default_args={"owner": "airflow", "depends_on_past": False, "retries": 1},
    schedule=None,  # Pas de schedule automatique
    start_date=datetime(2026, 1, 1),
    catchup=False,
    tags=["rattrapage", "manual"],
) as dag:

    # 1. Récupérer le payload passé lors du trigger manuel
    @task
    def get_manual_payload(**kwargs):
        ctx = get_current_context()
        if "dag_run" not in ctx or not ctx["dag_run"].conf:
            raise ValueError("No payload provided in dag_run.conf")
        return ctx["dag_run"].conf

    raw_payload = get_manual_payload()
    payload = validate_rattrapage_payload(raw_payload)
