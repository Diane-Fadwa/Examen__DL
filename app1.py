...
with DAG(
    ...
    def process_file(contract_path: str, file_path: str):
        ...
        def decompress_if_needed(file_path: str) -> str:
            ...
            # Nettoyage du chemin pour hcompressor
            # hcompressor attend un chemin absolu sans hdfs://nameservice1
            if file_path.startswith("hdfs://nameservice1"):
                machine_input_path = file_path[len("hdfs://nameservice1"):]
            elif file_path.startswith("hdfs://"):
                machine_input_path = file_path[len("hdfs://"):]
            else:
                machine_input_path = file_path

            # Supprime le slash initial si présent
            if machine_input_path.startswith("/"):
                machine_input_path = machine_input_path[1:]
            ...
            # Reconstruction du path HDFS complet pour Spark
